...
#include <QMessageBox>
#include <QLabel>
dehumidification::dehumidification(QWidget *parent) : QMainWindow(parent)
{
    this->setFixedSize(1000,600);
    this->setWindowTitle("编辑除湿系统信息页面");
    this->setWindowIcon(QPixmap(":/image/10.png"));
    //设置字体
    QFont qfont;
    ...
    //连接信号量
    connect(this,SIGNAL(senthumidity(int)),runface1,SLOT(rechumidity2(int)));
    connect(this,SIGNAL(sentPower(int)),runface1,SLOT(recPower5(int)));
    ...
    label->setText(QString("当前湿度：%1RH 当前功率：%2W").arg(deh.gethumidity()).arg(deh.getPower()));
    ...
    connect(confirmbtn1,&QPushButton::clicked,this,[=](){
        deh.sethumidity(line1->text().toInt());
        ...
        message.exec();
        label->setText(QString("当前湿度：%1RH 当前功率：%2W").arg(deh.gethumidity()).arg(deh.getPower()));
        emit senthumidity(deh.gethumidity());
    });
    ...
    connect(confirmbtn2,&QPushButton::clicked,this,[=](){
        ...
        message.exec();
        label->setText(QString("当前湿度：%1RH 当前功率：%2W").arg(deh.gethumidity()).arg(deh.getPower()));
        emit sentPower(deh.getPower());
    });
}
void dehumidification::paintEvent(QPaintEvent*)
{
    QPainter painter(this);
    QPixmap pix;
    pix.load(":/image/11.png");
